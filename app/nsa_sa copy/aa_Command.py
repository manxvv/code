...
class aa_Command(Script):
    ...
    def special_formate_scripts(self):
        ...
        self.s_dict['cli'] = [
            F'##########################- Pre Check -##########################',
            F'####---- NodeStatus ----####',
            F'pre_status_{self.usid.circle}_{self.usid.enm}.txt',
            F'',
            F'cmedit get {all_nodes} {status_command} --list',
            F'',
            F'####---- NodeAlarms ----####',
            F'pre_alarms_{self.usid.circle}_{self.usid.enm}.txt',
            F'',
            F'alarm get {all_nodes} --list',
            F'',
            F'####---- MO Dump ----####',
            F'pre_dump_{self.usid.circle}_{self.usid.enm}.txt',
            F'',
            F'cmedit get {all_nodes} {";".join([F"{_}.<w>" for _ in self.usid.dump_list])} --dynamic',
            F'',
            F'##########################- Activity -##########################',
            F'####---- Activity ----####',
            F'',
            ...
            F'7. Restart the NR Nodes base on guidelines',
            ...
            F'',
            F'####---- NodeAlarms ----####',
            F'post_alarms_{self.usid.circle}_{self.usid.enm}.txt',
            F'',
            F'alarm get {all_nodes} --list',
            F'',
            F'####---- MO Dump ----####',
            F'post_dump_{self.usid.circle}_{self.usid.enm}.txt',
            F'',
            F'alarm get {all_nodes} {";".join([F"{_}.<w>" for _ in self.usid.dump_list])} --dynamic',
            F'',
